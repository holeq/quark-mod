...
class Options_Framework_Admin {
    ...
    function settings_init() {
		...
		//Результат функции валидации validate_options будет сохранен в таблице wp_options в одном параметре $name.
	...
	function validate_options( $input ) {
	    ...
	    //$input содержит массив состоящий из значений параметров переданных $_POST`ом.
	    //var_dump($input);
		...
		foreach ( $options as $option ) {
			...
			//Плюс если вдруг в описанных опциях (options.php) будет параметр который отсутствует в параметрах полученных из $_POST, 
			//то этому параметру будет присвоено пустое значение (пустая строка).
			if ( has_filter( 'of_sanitize_' . $option['type'] ) ) {
				$clean[$id] = apply_filters( 'of_sanitize_' . $option['type'], $input[$id], $option );
			}
		}

		//Хук для передачи массива после его валидации.
		//Помним, что action никак не модифицирует переданный объект, а лишь дает поработать с ним на чтение.
