	...
	static function optionsframework_fields() {
		...
		foreach ( $options as $value ) {
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				...
				$output .= '<div id="' . esc_attr( $id ) .'" class="' . esc_attr( $class ) . '"'.(isset($value['cond-show']) ? ' cond-show="'.$value['cond-show'].'"' : '').'>'."\n";
				//$output .= '<div id="' . esc_attr( $id ) .'" class="' . esc_attr( $class ) . '"'.(isset($value['cond-show']) ? ' cond-show="'.htmlspecialchars(json_encode($value['cond-show'])).'"' : '').'>'."\n";
				if ( isset( $value['name'] ) ) {
			...
			/*
			if ( $value['id'] == "bo_page_5079_block5") {
			    echo $val;
			    exit;
			}
            */
			...
			case 'select':
				...
				if ( $value['id'] == "bo_page_5079_block5") {
				    $str = '';
				    foreach ($value['options'] as $key => $option ) {
    					$str .= '<option'. selected( $val, $key, false ) .' value="' . esc_attr( $key ) . '">' . esc_html( $option ) . '</option>';
    				}
    			    echo $str;
    			    exit;
    			}
			...
			if ( ( $value['type'] != "heading" ) && ( $value['type'] != "info" ) ) {
				...
				if ( ( $value['type'] != "checkbox" ) && ( $value['type'] != "editor" ) ) {
					...
					//BO-модификация для того, чтобы можно было использовать некторые HTML-теги в desc опций.
